...
class TSocket(TSocketBase):
  ...
  def generateGIHeader(self):
    clientId = 'PythonSDK\0'
    serviceTag = 'api\0'
    protocol = 'T'
    platform = 6
    protver = 0x00010000 | 100
    seqId = 0
    values = (clientId, serviceTag, protocol, platform, protver, seqId)
  ...
  def write(self, buff):
    ...
    sent = 0
    print "Buffer on TSocket.write entry: " + buff
    giheader = self.generateGIHeader()
    buff = giheader + buff
    have = len(buff)
    print "Injecting GI headers: " + giheader
    print "GI Header length: " + str(len(giheader))
    print "Full Buffer: " + buff
    print "Total Length: " + str(have)
    while sent < have:
